...
POST /generate — multi-category format:
{
  "brand": "AIWA",
  "categories": [
    {
  ...
  "output_filename": "AIWA_Catalogue_20260325.pptx"
}

Legacy single-category format still supported:
{
  "brand": "AIWA",
...
def populate_line_sheet(slide, category: str, products: list, prs: Presentation):
    """
    Fill a cloned line-sheet slide with product data.
    If fewer than 6 products, unused columns and images are removed
    and the remaining content is centered on the slide using the
    original column width (no stretching).
    """
    ...
    if is_partial:
        ...
        # Read original column width BEFORE removing anything
        grid           = tbl_xml.find(f'{{{NS_A}}}tblGrid')
        grid_cols      = grid.findall(f'{{{NS_A}}}gridCol')
        orig_col_width = int(grid_cols[0].get('w'))
        ...
        # Shrink table to fit only the remaining columns (no stretching)
        new_col_width     = orig_col_width
        new_table_width   = new_col_width * num_products
        table_shape.width = new_table_width

        # Center the table on the slide
        table_shape.left = (prs.slide_width - new_table_width) // 2

    # 5. Replace images (this resizes each shape to fit the image aspect ratio)
    pictures_now = sorted(
        [s for s in slide.shapes if s.shape_type == 13],
        key=lambda s: s.left
    )
...
def build_catalogue(prs: Presentation, categories: list) -> Presentation:
    """
    Keep slides 1+2, then generate line sheet pages grouped by category.
    Each category gets one or more 6-up slides, labelled in the top right.
    """
    TEMPLATE_SLIDE_IDX = 2  # slide 3 = 6-up line sheet template

    # Capture original slide count BEFORE cloning so deletion range stays correct
    original_slide_count = len(prs.slides)

    # Clone one slide per chunk across all categories, in order
    new_slides = []
    for cat in categories:
        category_name = cat.get("name", "Products")
        products      = cat.get("products", [])
        for chunk in chunk_products(products):
            new_slide = clone_slide(prs, TEMPLATE_SLIDE_IDX)
            new_slides.append((new_slide, category_name, chunk))
    ...
    for new_slide, category_name, chunk in new_slides:
        populate_line_sheet(new_slide, category_name, chunk, prs)
...
def generate():
    ...
    filename = body.get("output_filename", f"{brand}_Catalogue.pptx")

    # Support both new multi-category and legacy single-category formats
    if "categories" in body:
        categories = body.get("categories", [])
    else:
        # Legacy format — wrap in categories structure so the rest of the
        # code doesn't need to know about the old format at all
        categories = [{
            "name":     body.get("category", "Products"),
            "products": body.get("products", [])
        }]

    if not categories or not any(c.get("products") for c in categories):
        return jsonify({"error": "No products provided"}), 400

    total_products = sum(len(c.get("products", [])) for c in categories)
    log.info(f"Generating: {brand} / {len(categories)} categories / {total_products} products")

    try:
        prs = get_template()
        prs = build_catalogue(prs, categories)
        url = upload_result(prs, filename)
        return jsonify({"url": url, "filename": filename, "product_count": total_products})
    except S3Error as e:
